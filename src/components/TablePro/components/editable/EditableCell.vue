...
export default defineComponent({
  ...
  setup(props) {
      ...
      if (component === 'NDatePicker') {
        if (isString(value)) {
          ...
          if (compProps.valueFormat) {
            ...
            value = value.map((item: string) => parseISO(item).getTime());
          }
        }
      }
    ...
    const getValues = computed(() => {
      ...
      const option = options.find((item: { value: any }) => `${item.value}` === `${value}`);
      ...
      if (component === 'NDatePicker') {
        if (isNumber(currentValueRef.value)) {
          ...
          if (compProps.valueFormat) {
            ...
            currentValueRef.value = currentValueRef.value.map((item: number | Date) => {
