...
import { fetchLogin, fetchUserInfo, loginApi } from '@/service';
...
export const useAuthStore = defineStore('auth-store', {
  ...
  actions: {
     ...
     * @param password - 密码
     */
    async login(username: string, password: string, validateCode: string, type?: 'pwd' | 'sms') {
      ...
      const { error, data } = await loginApi(Params);
      // const { data } = await fetchLogin(account, pwd);
      if (data) {
